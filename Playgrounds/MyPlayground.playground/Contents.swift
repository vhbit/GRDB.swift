...
migrator.registerMigration("foo") { db in
    try db.create(table: "foo") { t in t.column("id", .integer).primaryKey() }
}
...
// See Relation https://github.com/groue/GRDB.swift/blob/Joins/GRDB/QueryInterface/SQLJoinDefinition.swift#L146
struct JoinPair<Left: RowConvertible, Right: RowConvertible> : RowConvertible {
    ...
    static func all(leftTable: String, rightTable: String) -> AnyTypedRequest<JoinPair<Left, Right>> {
        ...
        return SQLRequest(
            ...
            .bound(to: JoinPair<Left, Right>.self)
            .adapted { db in try ScopeAdapter(["right": SuffixRowAdapter(fromIndex: db.columnCount(in: leftTable))])
        }
    }
}
